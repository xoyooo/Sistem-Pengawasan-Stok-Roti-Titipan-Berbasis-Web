    ...
    <div class="w-full max-w-4xl mx-auto p-4 md:p-8">
        <div class="bg-white rounded-lg shadow">
            ...
            <div class="p-4 md:p-8">
                <form action="{{ route('sales.stok.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf

                    {{-- Nama Toko --}}
                    <div>
                        <label for="nama_toko" class="block text-sm font-medium text-gray-700 mb-2">Nama Toko</label>
                        <select name="nama_toko" id="nama_toko" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500" required>
                    ...
                    <div>
                        ...
                        <input type="number" name="jumlah_roti" id="jumlah_roti" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500" placeholder="Masukkan jumlah roti" required>
                    </div>

                    {{-- Jumlah Sisa --}}
                    <div>
                        <label for="jumlah_sisa" class="block text-sm font-medium text-gray-700 mb-2">Jumlah Sisa</label>
                        <input type="number" name="jumlah_sisa" id="jumlah_sisa" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500" placeholder="Masukkan jumlah sisa roti" required>
                    </div>

                    {{-- Tanggal Pengantaran --}}
                    <div>
                        <label for="tanggal_pengantaran" class="block text-sm font-medium text-gray-700 mb-2">Tanggal Pengantaran</label>
                        <input type="date" name="tanggal_pengantaran" id="tanggal_pengantaran" class="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-green-500" required>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="border-2 border-dashed border-gray-300 rounded-md p-8 text-center hover:border-green-500 transition">
                            ...
                            <label for="foto_roti" class="cursor-pointer">
                                ...
                                <p class="text-sm font-medium">Klik untuk upload foto roti</p>
                    ...
                    {{-- Foto Jumlah Sisa --}}
                    <div>
                        <label for="foto_sisa" class="block text-sm font-medium text-gray-700 mb-2">Foto Jumlah Sisa</label>
                        <div class="border-2 border-dashed border-gray-300 rounded-md p-8 text-center hover:border-green-500 transition">
                            <input type="file" name="foto_sisa" id="foto_sisa" class="hidden" accept="image/*">
                            <label for="foto_sisa" class="cursor-pointer">
                                <svg class="w-10 h-10 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                                </svg>
                                <p class="text-sm font-medium">Klik untuk upload foto sisa roti</p>
                                <p class="text-xs text-gray-400 mt-1">Maksimum size: 2 MB</p>
                            </label>
                        </div>
...
<script>
    ...
    document.getElementById('foto_sisa').addEventListener('change', function(e) {
        const fileName = e.target.files[0]?.name || '';
        document.getElementById('file-name-sisa').textContent = fileName ? 'File: ' + fileName : '';
    });
</script>
@endsection
